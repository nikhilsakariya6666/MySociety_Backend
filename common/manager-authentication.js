    ...
    if (token) {
        ...
        jwt.verify(token, app.get('superSecret'), function (err, decoded) {
            if (err) {
                ...
                if (decoded && decoded.role && (decoded.userType.toLowerCase() === 'superadmin' || decoded.userType.toLowerCase() === 'manager')) {
                    req.decoded = decoded
                    next()
                } else {
                    return res.json({ status: CONSTANT.FAIL, message: CONSTANT.MESSAGE.AUTH_ACCESS_FAIL })
                }
            }
        })
    } else {
